
export { GE } from "./core/implement/GE";
export { ManagerNameSpaces } from "./util/enums/NameSpaces";
export {GameObject} from "./managers/gameobject/implement/data/GameObject";
export {config} from './configs'
export { Position2DComponent } from './components/position2D/implement/Position2DComponent';
export { Render2DComp } from './components/render2D/implement/Render2DComp'
export {  Position2DComponentInterface  } from './components/position2D/interface/Position2DComponentInterface'
export {InitConfigInterface} from "./core/interface/InitConfigInterface";
